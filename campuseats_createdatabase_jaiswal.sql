CREATE DATABASE  IF NOT EXISTS `campus_eats_Jaiswal` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci */ /*!80016 DEFAULT ENCRYPTION='N' */;
USE `campus_eats_Jaiswal`;
-- MySQL dump 10.13  Distrib 8.0.22, for Win64 (x86_64)
--
-- Host: localhost    Database: campus_eats
--
-- This is the SQL dump for the database campus_eats_jaiswal containing everything INCLUDING the advanced queries.
-- ------------------------------------------------------
...
INSERT INTO `ratings` (`overall_rating`, `order_id`) VALUES
(1, 13),
(2, 63),
(3, 39),
(2, 52),
...
INSERT INTO `restaurant_rating` (`food_rating`, `price_rating`, `restaurant_id`, `order_id`) VALUES
(4, 5, 37, 105),
(1, 4, 101, 29),
(1, 4, 45, 65),
(5, 5, 29, 52),
...
INSERT INTO `driver_rating` (`ontime_rating`, `courteous_rating`, `driver_id`, `order_id`) VALUES
(5, 2, 5, 72),
(2, 2, 1, 46),
(3, 4, 6, 25),
(3, 4, 4, 96),
...
-- Add a hybrid rating for both drivers and restaurants
ALTER TABLE driver_rating
ADD COLUMN hybrid_driver_rating DOUBLE
GENERATED ALWAYS AS ((ontime_rating + courteous_rating)/2) STORED;

ALTER TABLE restaurant_rating
ADD COLUMN hybrid_restaurant_rating DOUBLE
AS ((food_rating + price_rating)/2) STORED;

-- Defining an average restaurant Rating for each Restaurant in the table 'Restaurant' (creating new COLUMN)
ALTER TABLE `restaurant` ADD COLUMN `restaurant_rating` float DEFAULT NULL;
SET SQL_SAFE_UPDATES = 0;
UPDATE `restaurant` t1
SET t1.restaurant_rating = 
    (SELECT AVG(t2.hybrid_restaurant_rating)
     FROM restaurant_rating t2 
     where  t1.restaurant_id=t2.restaurant_id);
SET SQL_SAFE_UPDATES = 1; 

-- Defining an average driver Rating for each Driver in the table 'Driver' (COLUMN exists already)
SET SQL_SAFE_UPDATES = 0;
UPDATE `driver` t1
SET t1.rating = 
    (SELECT AVG(t2.hybrid_driver_rating)
     FROM driver_rating t2 
     where  t1.driver_id=t2.driver_id);
SET SQL_SAFE_UPDATES = 1; 

-- Stored Procedures for calculating MaxMinAvg Ratings for Drivers and Restaurants
CALL Driver_MaxMinAvg_Rating;
CALL Restaurant_MaxMinAvg_Rating;

-- Advanced View #1: Show Cars ost preferred by Top-Rated Drivers using "driver-rating"

SELECT *
FROM (SELECT table1.*, driver.rating
FROM (SELECT driver_id, vehicle_id, COUNT(*) AS frequency
FROM delivery
WHERE vehicle_id = (
    SELECT vehicle_id
    FROM delivery AS x
    ...
    ORDER BY COUNT(*) DESC
    LIMIT 1)
GROUP BY driver_id, vehicle_id) as table1
JOIN driver
ON table1.driver_id = driver.driver_id) AS table2
JOIN vehicle
ON table2.vehicle_id = vehicle.vehicle_id
ORDER BY rating DESC;


-- Advanced View #2: Show ratings of the restaurants generating highest revenue

SELECT SUM(total_price) as revenue, restaurant_id, overall_rating FROM
(SELECT orders.order_id, orders.restaurant_id, ratings.overall_rating, orders.total_price 
FROM orders
JOIN ratings
ON orders.order_id = ratings.order_id) as A
